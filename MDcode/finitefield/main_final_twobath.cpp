...
void initialize(vector<vector<float> > &pos, vector<vector<float> > &vel, vector<vector<float> > &acc, float &boxlength, float temperature, float massO, float massH);
void resume(vector<vector<float> > &pos, vector<vector<float> > &vel, vector<vector<float> > &acc, float &boxlength, float temperature, float massO, float massH);
...
vector<float> dG4_ij_ik_jk(float r12, float r13, float r23, float cosalphaijk, float zeta, float yeta, float lam);
...
void get_rotamers(vector<vector<vector<float> > > &vecrij_nn, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2);
void rotate(vector<vector<float> > &xyz, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2, float xyz_rotated[], int id_i, int id_j);
void shift_rotate(vector<vector<float> > &xyz, float xyz_shifted[][natoms][3], vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2, float xyz_rotated[][natoms][3], float boxlength, int id_i);
void get_Ewald(vector<vector<float> > &pos, vector<vector<float> > &w_pos, vector<vector<float> > &EO, vector<vector<float> > &EH, double boxlength, vector<float> Eext);
...
void get_dist(float rij[][natoms], float vecrij[][natoms][3], vector<vector<float> > &pos, float boxlength);
...
void backrotate_shift_reshape(vector<vector<vector<float> > > &wannier_mapped_rotated, vector<vector<float> > & wxyz, vector<vector<float> > & pos, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2);
void backrotate_shift(vector<vector<vector<float> > > &wannier_mapped_rotated, vector<vector<vector<float> > > & wxyz, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2);
...
void backrotate(vector<vector<float> > &f_rotated, vector<vector<float> > & f_original, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2);
...
int main(int argc,char *argv[]) {
    ...
    for (istep = 0; istep < nstep; istep++){
        ...
        vector<vector<vector<float> > > R1O (noxygen, vector<vector<float> >(3, vector<float>(3)));
        vector<vector<vector<float> > > R2O (noxygen, vector<vector<float> >(3, vector<float>(3)));
        vector<vector<vector<float> > > R1H (nhydrogen, vector<vector<float> >(3, vector<float>(3)));
        vector<vector<vector<float> > > R2H (nhydrogen, vector<vector<float> >(3, vector<float>(3)));
        
        get_rotamers(vecrij_nnO, R1O, R2O);  // get the rotamers
        get_rotamers(vecrij_nnH, R1H, R2H);
        

        shift_rotate(pos, vecrij, R1O, R2O, pos_rotatedO, boxlength, 0);  // rotate the nucleus coordinate
        shift_rotate(pos, vecrij, R1H, R2H, pos_rotatedH, boxlength, 1);
        ...
        backrotate_shift_reshape(wxyz_mapped_rotated, wxyz, pos, R1O, R2O); // rotate back the wannier position for the GT system
        backrotate_shift(wxyz_mapped_rotated, wxyz_mapped, R1O, R2O); // rotate back the wannier position for the GT system
        ...
        vector<float> Eext(3);
        for (int i = 0; i < 3; i++){
            Eeffct[i] = -total_dipole[i] / pow(boxlength, 3) * 4 * M_PI;
            Eext[i] = Eeffct[i];
        } 
        Eext[2] += Ez;
        ...
        rotate(EO, R1O, R2O, EO_rotated, 0, 0);  // rotate the field around O
        rotate(EH, R1O, R2O, EH_rotated, 0, 1);

        rotate(EO, R1H, R2H, EO_rotated_H, 1, 0); // rotate the field around H
        rotate(EH, R1H, R2H, EH_rotated_H, 1, 1);
        ...
        while (wxyz_diff > 0.0005){
            ...
            //vector<vector<float> > wxyz(nwannier, vector<float>(3));
            backrotate_shift_reshape(wxyz_mapped_rotated_total, wxyz, pos, R1O, R2O);
            backrotate_shift(wxyz_mapped_rotated_total, wxyz_mapped, R1O, R2O);
            vector<float> total_dipole = get_dipole(wxyz_mapped, vecrij_nnO);
            vector<float> Eeffct(3);
            vector<float> Eext(3);
            for (int i = 0; i < 3; i++){
        ...
        compute_force_GT(rij, vecrij, fGT, massO, massH, boxlength, parameters2_OO, parameters2_OH, parameters2_HO, parameters2_HH, parameters4_OOO, parameters4_OOH, parameters4_OHH, parameters4_HHO, parameters4_HOO, xO_scaling, xH_scaling, net_forceGT);
        backrotate(fO_peturb, fO_peturb_backrotate, R1O, R2O);
        backrotate(fH_peturb, fH_peturb_backrotate, R1H, R2H);
...
void pbc(vector<vector<float> > &pos, float boxlength){
    ...
    for (int i = 0; i < natoms; i++)
    {
        for (int j = 0; j < 3; j++)
        {
            pos[i][j] = pos[i][j] - round(pos[i][j] / boxlength) * boxlength;
        }
    }
}
            ...
            if ((j != i) && (r[i][j] < rc))
            {
                for (int k = rangek[0]; k < rangek[1]; k++)
                {
                    if ((k != j) && (k != i) && (r[k][i] < rc) && (r[k][j] < rc))
                    {
                        ...
                        for (int ip = 0; ip < nx; ip++)
                        {
                            vector<float> Gs = dG4_ij_ik_jk(r[i][j], r[i][k], r[j][k], cosijk, parameters[ip][3], parameters[ip][1], parameters[ip][2]);
                            float dG4_ij_ij = Gs[0];
                            float dG4_ij_ik = Gs[1];
                            float dG4_jk_jk = Gs[2];
                            for (int ix = 0; ix < 3; ix++)
                            {
...
void get_rotamers(vector<vector<vector<float> > > &vecrij_nn, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2){
    vector<float> z_axis {0, 0, 1};
    vector<float> y_axis {0, 1, 0};
    for (int i = 0; i < vecrij_nn.size(); i++){
        ...
        vector<vector<float> > Ro1 = get_rotation(vec_nn1, z_axis);
        vector<float> vec_nn2p (3);
        for (int j = 0; j < 2; j++){
            for (int k =0 ; k< 3; k++){
                vec_nn2p[j] += Ro1[j][k] * vec_nn2[k];
            }
        }

        vector<vector<float> > Ro2 = get_rotation2(vec_nn2p, y_axis);

        for (int j = 0; j < 3; j++){
            for (int k = 0; k < 3; k++){
                rotamers1[i][j][k] = Ro1[j][k];
                rotamers2[i][j][k] = Ro2[j][k];
            }
        }
    }
            ...
            for (int k=0; k<3; k++){
                ...
                for (int jj=0; jj<3; jj++){
                    for (int kk=0; kk<3; kk++){
                        xyz_rotated[i * 3 * ny + j*3 + k] += rotamers2[i][k][jj] * rotamers1[i][jj][kk] * xyz[j][kk];
                    }
                }
    ...
    for (int i=0; i<nx; i++){
        ...
        for (int j=0; j<natoms; j++){
            for (int k=0; k<3; k++){
                ...
                for (int jj=0; jj<3; jj++){
                    for (int kk=0; kk<3; kk++){
                        xyz_rotated[i][j][k] += rotamers2[i][k][jj] * rotamers1[i][jj][kk] * xyz_shifted[ii][j][kk];
                    }
                }
...
void get_Ewald(vector<vector<float> > &pos, vector<vector<float> > &w_pos, vector<vector<float> > &EO, vector<vector<float> > &EH, double boxlength, vector<float> Eext){
    ...
    double coeff = 2 * M_PI / pow(boxlength, 3);
    #pragma omp parallel for
    for (int i = 0; i < kdim; i++){
        for (int j = 0; j < noxygen; j++){
            float k_r_prod = 0;
            ...
            for (int k=0; k<3; k++){
                dSkO_real[j][i][k] += - kxyz[i][k] * sin(k_r_prod);
                dSkO_imag[j][i][k] += kxyz[i][k] * cos(k_r_prod);
            }
        }
    }
    ...
    #pragma omp parallel for
    for (int i = 0; i < kdim; i++){
        for (int j = 0; j < nhydrogen; j++){
            float k_r_prod = 0;
            for (int k=0; k<3; k++){
                k_r_prod += kxyz[i][k] * pos[j + noxygen][k]; 
            }

            for (int k=0; k<3; k++){
                dSkH_real[j][i][k] += - kxyz[i][k] * sin(k_r_prod);
                dSkH_imag[j][i][k] += kxyz[i][k] * cos(k_r_prod);
            }
        }
    }
    ...
    for (int i =0;  i<noxygen; i++){      
        for (int k = 0; k < 3; k++){
            ...
            for (int j = 0; j < kdim; j++){
                EO[i][k] += -coeff * exp(-pow(sigmaE * knorm[j], 2)/2)/ pow(knorm[j], 2) * 2 * (Sk_real[j] * dSkO_real[i][j][k] + Sk_imag[j] * dSkO_imag[i][j][k]);
            }
            EO[i][k] += Eext[k];
        } 
    }
    ...
    for (int i =0;  i<nhydrogen; i++){      
        for (int k = 0; k < 3; k++){
            EH[i][k] = 0;
            for (int j = 0; j < kdim; j++){
                EH[i][k] += -coeff * exp(-pow(sigmaE * knorm[j], 2)/2)/ pow(knorm[j], 2) * 2 * (Sk_real[j] * dSkH_real[i][j][k] + Sk_imag[j] * dSkH_imag[i][j][k]);
            }
            EH[i][k] += Eext[k];
        } 
    }
...
void get_dist(float rij[][natoms], float vecrij[][natoms][3], vector<vector<float> > &pos, float boxlength){
    ...
    for (int i = 0; i < natoms; i++)
    {
        for (int j = 0; j < natoms; j++)
        {
            float disx =  pos[j][0] -  pos[i][0] - round(( pos[j][0] -  pos[i][0]) / boxlength) * boxlength;
            float disy =  pos[j][1] -  pos[i][1] - round(( pos[j][1] -  pos[i][1]) / boxlength) * boxlength;
            float disz =  pos[j][2] -  pos[i][2] - round(( pos[j][2] -  pos[i][2]) / boxlength) * boxlength;
    ...
    for (int i=0; i<noxygen; i++){
        for (int j=0; j<4; j++){
            for (int k=0; k<3; k++){
                for (int jj=0; jj<3; jj++){
                    for (int kk=0; kk<3; kk++){
                        wxyz[nacount][k] += wannier_mapped_rotated[i][j][jj] * rotamers2[i][jj][kk] * rotamers1[i][kk][k];
                    }
                }
    ...
    for (int i=0; i<noxygen; i++){
        for (int j=0; j<4; j++){
            for (int k=0; k<3; k++){
                for (int jj=0; jj<3; jj++){
                    for (int kk=0; kk<3; kk++){
                        wxyz[i][j][k] += wannier_mapped_rotated[i][j][jj] * rotamers2[i][jj][kk] * rotamers1[i][kk][k];
                    }
                }
...
void backrotate(vector<vector<float> > &f_rotated, vector<vector<float> > & f_original, vector<vector<vector<float> > > &rotamers1, vector<vector<vector<float> > > &rotamers2){
    ...
    for (int i=0; i<f_original.size(); i++){
        for (int k=0; k<3; k++){
            ...
            for (int jj=0; jj<3; jj++){
                for (int kk=0; kk<3; kk++){
                    f_original[i][k] += f_rotated[i][jj] * rotamers2[i][jj][kk] * rotamers1[i][kk][k];
                }
            }
