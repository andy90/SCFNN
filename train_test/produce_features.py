...
def features_wannier_GT(vecrij_OO, vecrij_OH):
    ...
    features_G2OO = np.zeros((parameters2_OO.shape[0], noxygen, nconfigs), dtype=np.float32)
    features_G2OH = np.zeros((parameters2_OH.shape[0], noxygen, nconfigs), dtype=np.float32)

    parameters4_OO = np.loadtxt("wG4_parameters_OO.txt")
    parameters4_OH = np.loadtxt("wG4_parameters_OH.txt")

    features_G4OO_zax = np.zeros((parameters4_OO.shape[0], noxygen, nconfigs), dtype=np.float32)
    features_G4OH_zax = np.zeros((parameters4_OH.shape[0], noxygen, nconfigs), dtype=np.float32)

    features_G4OO_xax = np.zeros((parameters4_OO.shape[0], noxygen, nconfigs), dtype=np.float32)
    features_G4OH_xax = np.zeros((parameters4_OH.shape[0], noxygen, nconfigs), dtype=np.float32)

    features_G4OO_yax = np.zeros((parameters4_OO.shape[0], noxygen, nconfigs), dtype=np.float32)
    features_G4OH_yax = np.zeros((parameters4_OH.shape[0], noxygen, nconfigs), dtype=np.float32)
    ...
    np.savetxt("wannier_GT_feature_scale.txt", scalefactor)
...
def features_wannier_peturb(vecrij_OO, vecrij_OH, EO, EH):
    ...
    # features_G2OO = np.zeros((parameters2_OO.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)
    # features_G2OH = np.zeros((parameters2_OH.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_zax = np.zeros((parameters4_OO.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_zax = np.zeros((parameters4_OH.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_xax = np.zeros((parameters4_OO.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_xax = np.zeros((parameters4_OH.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_yax = np.zeros((parameters4_OO.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_yax = np.zeros((parameters4_OH.shape[0], noxygen, 1, nfolders, nconfigs), dtype=np.float32)

    # features for those fields, we have additional dimension (the third one) which records the xyz of the E field
    features_EG2OO = np.zeros((parameters2_OO.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)
    features_EG2OH = np.zeros((parameters2_OH.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_zax = np.zeros((parameters4_OO.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_zax = np.zeros((parameters4_OH.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_xax = np.zeros((parameters4_OO.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_xax = np.zeros((parameters4_OH.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_yax = np.zeros((parameters4_OO.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_yax = np.zeros((parameters4_OH.shape[0], noxygen, 3, nfolders, nconfigs), dtype=np.float32)
    ...
    #     (parameters4_OH.shape[0] * 3, noxygen, 3, nconfigs))
...
def features_force_peturb(vecrij_OO, vecrij_OH, EO, EH, parameters2_OO, parameters2_OH, parameters4_OO, parameters4_OH):
    ...
    # features_G2OO = np.zeros((parameters2_OO.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)
    # features_G2OH = np.zeros((parameters2_OH.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_zax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_zax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_xax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_xax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)

    # features_G4OO_yax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)
    # features_G4OH_yax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 1, nfolders, nconfigs), dtype=np.float32)

    # features for those fields, we have additional dimension (the third one) which records the xyz of the E field
    features_EG2OO = np.zeros((parameters2_OO.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)
    features_EG2OH = np.zeros((parameters2_OH.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_zax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_zax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_xax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_xax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)

    # features_EG4OO_yax = np.zeros((parameters4_OO.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)
    # features_EG4OH_yax = np.zeros((parameters4_OH.shape[0], rij_OO.shape[0], 3, nfolders, nconfigs), dtype=np.float32)
    ...
    features_EG2OO_new = np.transpose(features_EG2OO, axes=(0, 2, 1, 3, 4)).reshape(
        (parameters2_OO.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    features_EG2OH_new = np.transpose(features_EG2OH, axes=(0, 2, 1, 3, 4)).reshape(
        (parameters2_OH.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OO_H1O_new = np.transpose(features_EG4OO_zax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OO.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OO_xax_new = np.transpose(features_EG4OO_xax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OO.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OO_yax_new = np.transpose(features_EG4OO_yax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OO.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OH_H1O_new = np.transpose(features_EG4OH_zax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OH.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OH_xax_new = np.transpose(features_EG4OH_xax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OH.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
    # features_EG4OH_yax_new = np.transpose(features_EG4OH_yax, axes=(0, 2, 1, 3, 4)).reshape(
    #     (parameters4_OH.shape[0] * 3, rij_OO.shape[0], 3, nconfigs))
