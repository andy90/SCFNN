...
wxyz_stack = np.stack(wxyz_all, axis=-1).reshape((nwannier,  3, nfolders, nconfigs_test))
Oxyz_stack = np.stack(Oxyz_all, axis=-1).reshape((noxygen, 3, nfolders, nconfigs_test))
Hxyz_stack = np.stack(Hxyz_all, axis=-1).reshape((nhydrogen, 3, nfolders, nconfigs_test))
boxlength_stack = np.stack(boxlength_all).reshape((nfolders, nconfigs_test))

fO_stack = np.stack(fO_all, axis=-1).reshape((noxygen, 3, nfolders, nconfigs_test))
fH_stack = np.stack(fH_all, axis=-1).reshape((nhydrogen, 3, nfolders, nconfigs_test))


np.save("wxyz_testconfigs", wxyz_stack)
np.save("Oxyz_testconfigs", Oxyz_stack)
np.save("Hxyz_testconfigs", Hxyz_stack)
np.save("boxlength_testconfigs", boxlength_stack)
np.save("fO_testconfigs", fO_stack)
np.save("fH_testconfigs", fH_stack)
...
def assemble_features(feature_name,  id_center):
    ...
    np.save("testfeatures_" + feature_name, features_all)
    np.save("testfeatures_d" + feature_name, features_d_all)
...
def assemble_features_further(feature_name_tuple):  # assemble features that have the same center atom
    ...
    for feature_name in feature_name_tuple:
        features = np.load("testfeatures_" + feature_name + ".npy")
        features_d = np.load("testfeatures_d" + feature_name + ".npy")
        features_all += (features,)
...
np.save("test_xO", xO)
np.save("test_xH", xH)
np.save("test_xOO_d", xOO_d)
np.save("test_xHO_d", xHO_d)
np.save("test_xHH_d", xHH_d)
np.save("test_xOH_d", xOH_d)
